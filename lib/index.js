const disableStylelintStylisticRules = () => {
  const stylisticConfig = require('@stylistic/stylelint-config');
  const disabledRules = {};
  const rules = stylisticConfig.default?.rules || {};

  // Additional rules that are not part of config, but we want disabled them anyway
  ...
  for (const rule of [...Object.keys(rules), ...additionalRules]) {
    disabledRules[rule] = null;
  }
...
module.exports = {
  ...
  rules: {
    ...
    ...disableStylelintStylisticRules(),
  },
};
